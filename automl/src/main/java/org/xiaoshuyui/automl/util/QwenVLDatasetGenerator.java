...
public class QwenVLDatasetGenerator {
  ...
  public QwenVLDatasetGenerator(
      S3FileDelegate s3FileDelegate, S3ConfigProperties s3ConfigProperties) {
  ...
  public String generateDataset(String annotationSavePath, String datasetSavePath)
      throws Exception {
    List results = new ArrayList<>();
    List<String> annotationFiles =
        s3FileDelegate.listFiles(annotationSavePath, s3ConfigProperties.getDatasetsBucketName());
    List<String> imageFiles =
        s3FileDelegate.listFiles(datasetSavePath, s3ConfigProperties.getDatasetsBucketName());

    List<SimpleEntry<String, String>> simpleEntries =
        FileAnnotationMerger.mergeFilesAndAnnotations(imageFiles, annotationFiles);
    ...
    for (SimpleEntry<String, String> simpleEntry : simpleEntries) {
      ...
      String annotationContent =
          s3FileDelegate.getFile(
              simpleEntry.getValue(), s3ConfigProperties.getDatasetsBucketName());
