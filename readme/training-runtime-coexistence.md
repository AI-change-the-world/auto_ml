...
| 上传的自定义模型 ZIP（训练脚本 + 可选初始权重）、平台数据集/标注集 | `model_training_runtime` | `training.code.execute` | 仅合规 ONNX |
| 上传的自定义模型 ZIP（训练脚本 + 可选初始权重）、脚本自行下载或生成数据 | `model_training_runtime` | `training.code.execute` | 仅合规 ONNX |

自定义模型 ZIP 是完整的训练版本，不是旧训练器的基础模型。因此它不进入
`base_models`，也不会出现在内置训练的模型选择中。ZIP 必须包含训练脚本，可选
包含初始权重；两者同版本上传，同样不交给 `model_trainer`。

自定义脚本产生的检查点也不会出现在内置训练的“历史继续训练”候选中。若要继续
自定义训练，将该 checkpoint 随新版本自定义模型 ZIP 一起上传，并由脚本自行加载。

## 用户操作

在“模型管理”的“自定义模型”页签导入一个 ZIP：

- 根目录包含 `training_package.json` 和训练入口，例如 `train.py`，声明运行时、
  支持任务、参数、输入模式和输出产物。
- 若要从预训练权重继续训练，把权重文件一同放进 ZIP。运行时安全解压 ZIP 后，脚本
  通过 `Path(__file__).resolve().parent / "weights" / "initial.pt"` 等自定义相对
  路径读取它。
- 脚本自行判断该文件是否存在：存在则加载，不存在便按自己的逻辑从零训练；不需要
  manifest 字段或第二次上传。
...
当前只接受与现有分类或检测推理模板兼容的任务类型。`.pt` 权重和训练检查点可
随新的自定义模型 ZIP 使用，但不能直接进入部署列表。
