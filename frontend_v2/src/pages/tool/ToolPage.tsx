...
const ToolPage: React.FC = () => {
    ...
    const handleCreate = async () => {
        if (!form.name) { message.warning(tc('msg.pleaseInputName')); return; }
        setCreating(true);
        try {
    ...
    return (
        <div className="page-container">
            ...
            <Modal title={t('addTitle')} open={createOpen} onOk={handleCreate} onCancel={() => setCreateOpen(false)} confirmLoading={creating} okText={tc('action.create')} cancelText={tc('action.cancel')}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginTop: 16 }}>
                    <div>
                        <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: '#555', marginBottom: 4 }}>{tc('label.name')}</label>
                        <Input placeholder={t('modelName')} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
                    </div>
                    <div>
                        <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: '#555', marginBottom: 4 }}>{t('modelType')}</label>
                        <Select style={{ width: '100%' }} placeholder={t('selectType')} value={form.model_type || undefined} onChange={(v) => setForm({ ...form, model_type: v })} allowClear
                            options={[
                                { label: t('detection'), value: 'detection' },
                                { label: t('segmentation'), value: 'segmentation' },
                                { label: t('classification'), value: 'classification' },
                                { label: t('ocr'), value: 'ocr' },
                            ]} />
                    </div>
                    <div>
                        <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: '#555', marginBottom: 4 }}>{t('endpoint')}</label>
