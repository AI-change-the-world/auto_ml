...
class NacosConfigCenter:
    ...
    def refresh(self, notify: bool = True) -> dict:
        ...
        try:
            ...
            if not self._last_fetch_failed:
                logger.warning(f"Failed to refresh Nacos config: {e}")
    ...
    def start(self):
        ...
        self._started = True
        logger.info(f"Nacos config sync started, mode={self._listener_mode}")
    ...
    def _start_listener(self) -> bool:
        ...
        self._listener_thread = threading.Thread(
            ...
            name="trainer-nacos-config-listener",
            daemon=True,
        )
    ...
    async def _listener_main(
        ...
        try:
            ...
            async def on_change(tenant, changed_data_id, changed_group, content):
                logger.info(
                    f"Nacos listener received update: dataId={changed_data_id}, group={changed_group}"
                )
            ...
            logger.warning(f"Nacos listener failed, falling back to polling: {e}")
            self._listener_mode = "poll"
            self._start_polling()
        finally:
    ...
    def _start_polling(self):
        ...
        self._poll_thread = threading.Thread(
            ...
            name="trainer-nacos-config-poller",
            daemon=True,
        )
        self._poll_thread.start()
        self._listener_mode = "poll"
        logger.info(
            f"Nacos config polling started, interval={self.poll_interval()}s"
        )
    ...
    def _apply_config_text(self, config_text: str, notify: bool = True) -> dict:
        ...
        if changed and notify:
            logger.info(f"Nacos config updated, keys={list(config_data.keys())}")
            for name, callback in callbacks.items():
                try:
                    callback(old_config, config_data)
                except Exception as e:
                    logger.error(f"Nacos config callback '{name}' failed: {e}")
        return config_data
