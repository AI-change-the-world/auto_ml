...
class Annotation(BaseEntity):
    ...
    annotation_type = Column(Integer, default=0, comment="标注类型: 0=检测, 1=分类, 2=分割")
    classes = Column(Text, nullable=True, comment="分类项 JSON")
    storage_type = Column(Integer, default=1, comment="存储类型: 0=本地, 1=S3, 2=WebDAV")
    save_path = Column(String(512), nullable=True, comment="存储路径")
    prompt = Column(Text, nullable=True, comment="AI 标注提示词")
    dataset_id = Column(BigInteger, nullable=True, index=True, comment="关联数据集ID")
