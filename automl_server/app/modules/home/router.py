...
async def get_stats(db: AsyncSession = Depends(get_db)):
    ...
    running_tasks = (await db.execute(
        select(func.count()).select_from(Task).where(Task.is_deleted == False, Task.status == 1)
    )).scalar()
    
    completed_tasks = (await db.execute(
        select(func.count()).select_from(Task).where(Task.is_deleted == False, Task.status == 3)
    )).scalar()
