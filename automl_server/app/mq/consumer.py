...
class MessageConsumer:
    ...
    def _connect(self):
        ...
        try:
            ...
            logger.info(f"Consumer connected to RabbitMQ: {self.config.host}:{self.config.port}")
    ...
    def _setup_queues(self):
        ...
        queue_bindings = [
            ...
            (self.config.model_registered_queue, self.config.model_registered_routing_key),
            (self.config.model_deployed_queue, self.config.model_deployed_routing_key),
            (self.config.model_undeployed_queue, self.config.model_undeployed_routing_key),
        ]
    ...
    def register_handler(self, message_type: str, handler: Callable, is_async: bool = False):
        ...
        logger.info(f"Registered handler for {message_type} (async={is_async})")
