...
class InjectDataset(Dataset):
    def __init__(self, defect_dir, mask_dir, image_size=256):
        ...
        print(f"clean: {len(self.clean_paths)}  defect: {len(self.defect_paths)}  mask: {len(self.mask_paths)}" )
        self.transform = transforms.Compose([
            transforms.Resize((image_size, image_size)),
        ...
        self.mask_transform = transforms.Compose([
            transforms.Resize((image_size, image_size)),
            transforms.ToTensor()  # 不归一化
        ])

    def __len__(self):
        return min(len(self.clean_paths), len(self.defect_paths), len(self.mask_paths))
