...
| `model_training_runtime` | 实验性模型训练运行时；当前在单服务内执行已注册训练包，具备契约、数据物化、事件和结果校验能力，尚未替换 `model_trainer` | [README](./model_training_runtime/README.md) |
...
├── ai_pipeline_runtime/
├── ai_pipeline_sandbox/
├── model_training_runtime/  # 实验性；单服务内直接执行，尚未替换 model_trainer
├── mysql/
├── nacos/
├── readme/
├── docker-compose.yml
